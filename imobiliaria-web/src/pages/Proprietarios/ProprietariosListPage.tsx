...
.pill-link { display:inline-block; padding:6px 12px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; font-weight:600; font-size:14px; color:#0f172a; text-decoration:none; transition:background .2s ease }
    ...
    <div className="page">
      ...
      <section className="card">
        <div className="cardhead">
          ...
          {/* Botão no canto direito do card, igual "Adicionar imóvel" */}
              ...
              {loading ? (
                ...
                filtrados.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      <td>
                        <Link to={`/proprietarios/${slug}`} className="pill-link">{p.nome || "(sem nome)"}</Link>
                      </td>
                      <td>{p.doc ?? "-"}</td>
                      <td>{p.email ?? "-"}</td>
                      <td>{p.tel ?? "-"}</td>
