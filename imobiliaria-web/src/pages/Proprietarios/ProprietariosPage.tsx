  ...
  function AddProprietarioModal({
    ...
    onClose,
    onSaved,
    proprietario,           // proprietário existente para edição
    initialFocusField,      // campo para focar e "piscar"
  }: {
    open: boolean;
    onClose: () => void;
    ...
    async function salvar() {
      try {
        // normaliza -> envia null quando vazio
        const toNull = (v?: string | null) => {
          const t = (v ?? "").trim();
          return t ? t : null;
        };
        ...
        const payload: Proprietario = {
          // mesmo que o tipo peça string, mandamos null no JSON (backend aceita nulos)
    ...
    return (
      ...
      <div className="backdrop">
        {/* opcional: impedir propagação de cliques dentro do modal */}
        <div className="small-modal" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
  ...
  function ProprietariosListView() {
    ...
    function slugify(nome?: string | null, id?: number) {
      const base = (nome ?? "proprietario").toString().trim();
      const safe = base ? base.replace(/\s+/g, "-") : "proprietario";
      return `${encodeURIComponent(safe)}-${id ?? Math.floor(Math.random() * 10000)}`;
    }
  ...
  /* ============================================================
    ...
    ============================================================ */
/* ============================================================
   DETALHE — ProprietarioDetalheView
   ============================================================ */
  function ProprietarioDetalheView() {
    ...
    const abaAtual = isImoveis ? "Imóveis" : "Dados";
      ...
      <div className="page">
        ...
        <div className="header">
          <div className="breadcrumb">
            ...
            {owner ? (
              <>
                <Link to={base}>{owner.nome}</Link> /{" "}
                <span className="breadcrumb-active">{abaAtual}</span>
              </>
            ) : (
